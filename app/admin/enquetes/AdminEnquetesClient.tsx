...
export default function AdminEnquetesClient({ enquetes: enquetesIniciais }: { enquetes: Enquete[] }) {
  ...
  return (
    <main className="px-6 py-8 md:px-12 md:py-12 w-full">
        ...
        {editando !== null && (
          <Panel className="p-6 mb-8 border-mint">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-xl text-black">{editando ? 'Editar votação' : 'Nova votação'}</h2>
              <button onClick={() => setEditando(null)} className="text-gray-text hover:text-black">
                <X size={20} />
              </button>
            </div>
            <div className="grid gap-4 mb-6">
              <div>
                <label className="block text-xs font-medium text-black mb-2">Título *</label>
                <input type="text" value={form.titulo || ''} onChange={(e) => setForm({ ...form, titulo: e.target.value })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" placeholder="ex: Qual é o melhor dia?" />
              </div>
              <div>
                <label className="block text-xs font-medium text-black mb-2">Descrição</label>
                <textarea value={form.descricao || ''} onChange={(e) => setForm({ ...form, descricao: e.target.value })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" rows={3} placeholder="ex: Vote no sábado que funciona melhor para você" />
              </div>
              <div className="grid sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-medium text-black mb-2">Tipo *</label>
                  <select value={form.tipo || 'presencial'} onChange={(e) => setForm({ ...form, tipo: e.target.value as 'online' | 'presencial' })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep">
                    <option value="presencial">Presencial</option>
                    <option value="online">Online</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-black mb-2">Escolhas</label>
                  <select value={form.permitir_multiplas ? 'multipla' : 'unica'} onChange={(e) => setForm({ ...form, permitir_multiplas: e.target.value === 'multipla' })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep">
                    <option value="unica">Escolha única</option>
                    <option value="multipla">Múltiplas escolhas</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-black mb-2">Status</label>
                  <select value={form.ativo ? 'ativo' : 'inativo'} onChange={(e) => setForm({ ...form, ativo: e.target.value === 'ativo' })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-black mb-2">Início *</label>
                  <input type="date" value={form.data_inicio || ''} onChange={(e) => setForm({ ...form, data_inicio: e.target.value })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-black mb-2">Fim *</label>
                  <input type="date" value={form.data_fim || ''} onChange={(e) => setForm({ ...form, data_fim: e.target.value })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" />
                </div>
              </div>
              {form.tipo === 'presencial' && (
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-black mb-2">Horário</label>
                    <input type="text" value={form.horario || ''} onChange={(e) => setForm({ ...form, horario: e.target.value })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" placeholder="ex: 11:30 às 17h" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-black mb-2">Local</label>
                    <input type="text" value={form.local || ''} onChange={(e) => setForm({ ...form, local: e.target.value })} className="w-full px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" placeholder="ex: Pinheiros, São Paulo" />
                  </div>
                </div>
              )}
              <div>
                <div className="flex items-center justify-between mb-3">
                  <label className="block text-xs font-medium text-black">Alternativas *</label>
                  <button onClick={adicionarOpcao} type="button" className="flex items-center gap-1 text-xs font-medium text-mint hover:text-mint-dark">
                    <Plus size={14} />
                    Adicionar
                  </button>
                </div>
                <div className="space-y-2">
                  {opcoes.map((opcao, i) => (
                    <div key={i} className="flex gap-2">
                      <input type="text" value={opcao.texto} onChange={(e) => atualizarOpcao(i, e.target.value)} className="flex-1 px-4 py-2 border border-gray-faint rounded-lg focus:outline-none focus:border-brown-deep" placeholder={`Alternativa ${i + 1}`} />
                      <button onClick={() => removerOpcao(i)} type="button" className="text-gray-text hover:text-red-600 transition p-2">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ))}
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setEditando(null)} className="border border-brown-deep text-black hover:bg-brown-deep/10 font-medium px-6 py-2.5 rounded-lg transition-colors">
                Cancelar
              </button>
            </div>
          </Panel>
        ...
        <section>
          ...
          <div className="space-y-3">
            {enquetes.length === 0 ? (
              <Panel className="p-6 text-center text-gray-text">Nenhuma votação cadastrada</Panel>
            ) : (
              enquetes.map((enquete) => (
                <Panel key={enquete.id} className={`p-5 flex items-start justify-between gap-4 ${!enquete.ativo ? 'opacity-60' : ''}`}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <h3 className="font-medium text-black">{enquete.titulo}</h3>
                      ...
                      <span className="text-[10px] font-medium px-2 py-1 rounded-full uppercase tracking-wide bg-mint-light text-black">{enquete.tipo}</span>
                    ...
                    {enquete.descricao && <p className="text-sm text-gray-text mb-2">{enquete.descricao}</p>}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => iniciarEdicao(enquete)} className="text-gray-text hover:text-black transition p-2" title="Editar">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleToggleAtivo(enquete.id, enquete.ativo)} className="text-gray-text hover:text-black transition p-2" title={enquete.ativo ? 'Desativar' : 'Ativar'}>
                      <Check size={16} className={enquete.ativo ? 'text-green-600' : ''} />
                    </button>
                    <button onClick={() => handleDeletar(enquete.id)} disabled={deletando === enquete.id} className="text-gray-text hover:text-red-600 transition p-2 disabled:opacity-60" title="Deletar">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </Panel>
