...
export default function EnviarFeedbackClient({ mentorados }: { mentorados: Profile[] }) {
  ...
  return (
    <div className="bg-paper rounded-xl border border-line p-8">
      <h2 className="text-2xl font-medium text-brown-deep mb-6">Enviar Feedback ao Mentorado</h2>

      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-ink mb-2">Mentorado</label>
          <select
            value={mentoradoId}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-ink mb-2">Tipo</label>
            <select
              value={tipo}
          ...
          <div>
            <label className="block text-sm font-medium text-ink mb-2">URL do Arquivo (opcional)</label>
            <input
              type="url"
              value={arquivoUrl}
        ...
        <div>
          <label className="block text-sm font-medium text-ink mb-2">Conteúdo</label>
          <textarea
            value={conteudo}
            onChange={(e) => setConteudo(e.target.value)}
            placeholder="Escreva o feedback aqui..."
            className="w-full h-32 border border-line rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brown-deep resize-none"
          />
        </div>
